...
- **Unmatched egress falls back to the gateway.** If a tenant has no scoped table, or no entry matches, the request is forwarded downstream (to the gateway) rather than failing.
...
- **Missing endpoint is tolerated.** If mgr-applications returns 404/405 for `POST /modules/{id}/bootstrap` (older deployments without the endpoint), scoped egress is skipped and the sidecar continues forwarding egress to the gateway.
- **Per-tenant refresh failures are isolated.** A failed refresh for one tenant is logged and does not affect other tenants.
...
- **Depends on: mgr-applications** — `POST /modules/{id}/bootstrap` with `type=egress` (per-tenant application scope) and `type=ingress` (the module's own routes). See the mgr-applications `module-bootstrap` feature.
