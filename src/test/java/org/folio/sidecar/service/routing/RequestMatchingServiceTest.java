...
class RequestMatchingServiceTest {
  ...
  @Mock private ModuleProperties moduleInfo;
  @Mock private SidecarProperties sidecarProperties;

  @BeforeEach
  void setUp() {
    when(moduleInfo.getName()).thenReturn(TestConstants.MODULE_NAME);
    ...
    requestMatchingService = new RequestMatchingService(moduleInfo, sidecarProperties);
  ...
  void lookupForIngressRequest_parameterized(HttpMethod method, String path, ScRoutingEntry expected) {
    var actual = requestMatchingService.lookupForIngressRequest(routingContext(method, path));
  ...
  void lookupForIngressRequest_positive_modulePrefixEnabled() {
    ...
    var actual = requestMatchingService.lookupForIngressRequest(routingContext(GET, "/mod-foo/foo/entities"));
  ...
  void lookupForIngressRequest_positive_foundRouteWithModulePrefixEnabled() {
    when(sidecarProperties.isModulePrefixEnabled()).thenReturn(true);
    var actual = requestMatchingService.lookupForIngressRequest(routingContext(GET, "/mod-bar/foo/entities/1"));
  ...
  void lookupForEgressRequest_positive() {
    ...
    assertThat(requestMatchingService.egressRequestCache)
      .containsEntry(path, List.of(expectedScRoutingEntry));
  }
