...
public class ErrorHandler {
  ...
  private static final String MOD_USERS_TARGET_RETRY_DELAY = "5"; // in seconds
  ...
  private ErrHandler createHandler() {
    return ErrHandler.builder()
      ...
      .add(
        ModUsersTargetNotResolvedException.class, (cause, rc) ->
          sendErrorResponse(rc, cause, SERVICE_UNAVAILABLE, ErrorCode.MOD_USERS_TARGET_NOT_RESOLVED_ERROR,
            "mod-users-keycloak address is not resolved for the tenant yet. Retry later",
            Map.of(RETRY_AFTER, MOD_USERS_TARGET_RETRY_DELAY)))
      .add(
